...
def detect_regional_anomalies(df: pd.DataFrame) -> List[Dict]:
    ...
    # Clean region data - remove NaN
    df_clean = df[df['region'].notna() & (df['region'] != 'nan') & (df['region'] != '')].copy()

    if len(df_clean) == 0:
        return anomalies

    # Calculate high risk rate per site
    df_clean['high_risk_rate'] = df_clean['high_risk_count'] / df_clean['subject_count'].replace(0, 1)
...
def run_anomaly_detection():
    ...
    print(f"\n✅ Detected {len(stat_anomalies)} statistical outliers")
...
5. Run: python src/07_multi_agent_system.py
""")
